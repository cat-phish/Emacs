...
(with-eval-after-load 'org
  (defun org-cycle-hide-drawers (state)
    ...
    (when (and (derived-mode-p 'org-mode)
      ...
      (save-excursion
        (let* ((globalp (memq state '(all contents)))
               (beg (if globalp (point-min) (save-excursion (org-back-to-heading t) (point))))
               (end (if globalp (point-max) (save-excursion (org-end-of-subtree t)))))
          (org-hide-drawer-all beg end)))))
...
If GLOBAL is non-nil, apply to entire buffer."
    (interactive)
    (save-excursion
      (if global
          (progn
       ...
       (lambda ()
         (let ((state (org-get-todo-state)))
           (when (and state (member state org-done-keywords))
             (org-flag-subtree t) ;; Fold DONE/CANCELED
             ;; Skip checking children of folded subtree
     ...
     ((org-at-heading-p)
      (let ((state org-cycle-subtree-status))
        (cond
         ...
         (t
          (org-cycle)
          (org-cycle-hide-drawers 'subtree) ;; Always enforce drawer hiding
          ))))
     ;; Fallback
     (t
      (evil-next-line))))
  ...
  (defun my/org-cycle-dwim ()
    "TAB cycling with 5 states: folded -> children -> smart -> subtree (hidden) -> subtree (shown)."
    (interactive)
    (if (org-at-heading-p)
        (let ((state org-cycle-subtree-status))
          (cond
  ...
  (defun my/org-shifttab-dwim ()
    "S-TAB cycling with 5 states: overview -> contents -> smart -> all (hidden) -> all (shown)."
    (interactive)
    (cond
