...
These optimizations include (search terms in brackets):
...
- [[(agenda-overview)][Agenda overview]] has been made concise and readable.
    - All agenda items show their source headers inline.
    - High priority and NEXT items show at the top.
    - Daily overview shows Overdue items first, then items from today.
    - Upcoming shows items for the next 7 days.
    - Backlog shows only unscheduled TODOs specifically.
...
- [[(launch-done-collapse)][DONE]] items show as collapsed on launch to keep them out of the way.
