...
#+end_src
** About This Config
This is my personal Emacs config, it is built on top of the excellent
Kickstart.emacs project by MiniApollo. This is an Org Mode first config.
Although this config is set up for basic coding operations, I do not
personally write code in Emacs, so there are very few optimizations in
this config for that.

The goal for this config was to mirrror my personal Neovim config as
